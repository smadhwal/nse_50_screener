...
df_confirmed = df[df["Signal"] == "Confirmed Momentum"]
df_early = df[df["Signal"] == "Early Momentum"]
df_none = df[df["Signal"] == "No Signal"]
df_top20 = df_confirmed.sort_values("Momentum Score", ascending=False).head(20)

with pd.ExcelWriter("NSE250_Weekly_Momentum_Screener.xlsx", engine="openpyxl") as writer:
    df_top20.to_excel(writer, sheet_name="Top 20 Momentum", index=False)
...
print("Scan complete — trade levels restored successfully.")
